...
public interface CommonDaoInter<T extends BaseEntity<PK>, PK extends Serializable> {
	 ...
	 * @param properties 必须包含id 和 被更新的字段（key为字段名，value为对应的值）
	 * @return true:成功，false:失败
	 */
	boolean update(Map<String, Object> properties);
	...
	boolean update(T entity);
	  ...
	  * @return true:成功，false:失败
	  */
	boolean saveByUpsert(Map<String, Object> properties);
	
	/**
	 * 若entity有id且id在数据库对应的有记录，根据id则会覆盖之前的记录<br/>
	 * 若entity没有id 或者 id在数据库中没有对应的记录，则会插入新列(没有Id会生成Id，有则直接插入)
	 * @param entity 
	 * @return true:成功，false:失败
	 */
